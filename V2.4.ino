...
float DPF_LRD_calc_abs = 0;
float PM_GEN_calc_abs = 0;
...
DEBUG_PORT.println("CASE 5");
      ...
      if (start_up == false)
      {
      DPF_LRD_calc_avg = DPF_LRD_calc;
      PM_GEN_calc_avg = PM_GEN_calc;
      start_up = true;
      DEBUG_PORT.println("pierwszy wpis");
      }
      if (currentMillis - previousMillis2 >= 500) 
        {
          DEBUG_PORT.println(currentMillis);
          DPF_LRD_calc_abs = DPF_LRD_calc - DPF_LRD_calc_avg;
          DPF_LRD_calc_abs = abs(DPF_LRD_calc_abs);
          DEBUG_PORT.println(DPF_LRD_calc_abs);
             if (DPF_LRD_calc_abs < 1)
             {
             DPF_LRD_calc_avg = DPF_LRD_calc;
             DEBUG_PORT.println(DPF_LRD_calc_avg);
             }
             else if (DPF_Regen_b==2)
             {
             DPF_LRD_calc_avg = DPF_LRD_calc;
             DEBUG_PORT.println("case 5 regen");
             }
            
          PM_GEN_calc_abs = PM_GEN_calc - PM_GEN_calc_avg;
          PM_GEN_calc_abs = abs(PM_GEN_calc_abs);
          DEBUG_PORT.println(PM_GEN_calc_abs);
             if (PM_GEN_calc_abs < 0.2)
             {
             PM_GEN_calc_avg = PM_GEN_calc;
